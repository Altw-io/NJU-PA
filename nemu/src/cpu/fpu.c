...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	if ((sig_grs >> (23 + 3)) > 1 || exp < 0)
	{
		...
		while ((((sig_grs >> (23 + 3)) > 1) && exp < 0xff) // condition 1
			   ...
			   )// 一直到0101为止，0101末位已经是0，靠GRS还是能化成1的
		{
...
// 		assert(0);
		...
		if(GRS > 4)
		{
			...
			{
				exp++;
				if (exp >= 0xff) return sign ? N_INF_F : P_INF_F;
				sig_grs >>= 1;
			}
		}
		if(GRS == 4)
...
uint32_t internal_float_add(uint32_t b, uint32_t a)
{
	...
	//阶码为0，则尾数隐藏位为0，指数部分依然是2^(-126)。隐藏位显式为0后，阶码就从0设为1，以方便计算阶差
	//隐藏位显式为1，阶码为1，就是原先隐藏位不写时，阶码为1的规格化表达，即1.f*2^(-126)
	//隐藏位显式为0，阶码为1，就是原先的非规格化数，0.f*2^(-126)
	//此时-126和阶码为1完全划等号，计算阶差也要以重设阶码后的值计算
	sig_a = fa.fraction;
	if (fa.exponent != 0)
